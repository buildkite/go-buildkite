...
type ClusterQueue struct {
	...
	DispatchPausedBy   *User           `json:"dispatch_paused_by,omitempty" yaml:"dispatch_paused_by,omitempty"`
...
func (cqs *ClusterQueuesService) List(org, clusterID string,  opt *ClusterQueuesListOptions) ([]ClusterQueue, *Response, error) {
	...
	clusterQueues := new([]ClusterQueue)

	resp, err := cqs.client.Do(req, clusterQueues)

	if err != nil {
		return nil, resp, err
	}

	return *clusterQueues, resp, err
}
