package main

import (
	"context"
	"encoding/json"
	"log"

	"github.com/buildkite/go-buildkite/v5"

	"github.com/alecthomas/kingpin/v2"
)

var (
	apiToken     = kingpin.Flag("token", "API token").Required().String()
	org          = kingpin.Flag("org", "Organization slug").Required().String()
	registrySlug = kingpin.Flag("registry", "Registry Slug").Required().String()
	tokenID      = kingpin.Flag("token-id", "Registry Token ID").Required().String()
)

func main() {
	kingpin.Parse()

	client, err := buildkite.NewOpts(buildkite.WithTokenAuth(*apiToken))
	if err != nil {
		log.Fatalf("creating buildkite API client failed: %v", err)
	}

	token, _, err := client.PackageRegistryTokensService.Get(context.Background(), *org, *registrySlug, *tokenID)
	if err != nil {
		log.Fatalf("Getting registry token %s failed: %s", *tokenID, err)
	}

	data, err := json.MarshalIndent(token, "", "\t")
	if err != nil {
		log.Fatalf("json encode failed: %s", err)
	}

	log.Println(string(data))
}
